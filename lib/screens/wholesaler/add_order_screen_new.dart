...
class _AddOrderScreenState extends State<AddOrderScreen> {
  ...
  void _filterShopOwners(String query) {
    ...
    final filtered = allShopOwners.where((owner) {
      final shopName = owner['shop_name']?.toLowerCase() ?? '';
      final fullName = owner['full_name']?.toLowerCase() ?? '';
      final queryLower = query.toLowerCase();
      return shopName.contains(queryLower) || fullName.contains(queryLower);
    }).toList();

    setState(() {
      filteredShopOwners = filtered;
    });
  }
  ...
  Future<void> _submitOrder() async {
    ...
    try {
      ...
      final response = await WholesalerApiService.createOrder(
        ...
        subcategoryId: selectedInventoryItem!['id'],
        quantity: quantity,
        unitPrice: unitPrice,
      );
          ...
          children: [
            ...
            Stack(
              children: [
                ...
                if (filteredShopOwners.isNotEmpty && isSearchFocused)
                  Positioned(
                    top: 60,
                    left: 0,
                    right: 0,
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              ...
                              title: Text(
                                owner['shop_name'] ?? owner['full_name'] ?? 'N/A',
                                style: const TextStyle(fontWeight: FontWeight.w500),
                              ),
                              subtitle: Text(
                                '${owner['phone'] ?? 'No phone'} • ${owner['location'] ?? 'No location'}',
                              ),
