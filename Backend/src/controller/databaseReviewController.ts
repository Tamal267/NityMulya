...
export class DatabaseReviewController {
  ...
  async createProductReview(c: Context) {
    try {
      ...
      if (!customer_id || !customer_name || !customer_email || !subcat_id || !product_name || !shop_owner_id || !shop_name || !rating || rating < 1 || rating > 5) {
        return c.json({ 
          success: false,
      ...
      const [review] = await sql`
        INSERT INTO customer_product_reviews (
          ...
          ${subcat_id}, ${product_name},
          ${shop_owner_id}, ${shop_name},
          ${rating}, ${comment || ''}, ${order_id || null}, ${is_verified_purchase}
        ) RETURNING *
      `;
  ...
  async createShopReview(c: Context) {
    try {
      ...
      // Validate required fields
      if (!customer_id || !customer_name || !customer_email || !shop_owner_id || !shop_name || !overall_rating || overall_rating < 1 || overall_rating > 5) {
        return c.json({ 
          success: false,
      ...
      const [review] = await sql`
        INSERT INTO customer_shop_reviews (
          ...
          ${customer_id}, ${customer_name}, ${customer_email},
          ${shop_owner_id}, ${shop_name},
          ${overall_rating}, ${service_rating || overall_rating}, ${delivery_rating || overall_rating},
  ...
  async getProductReviews(c: Context) {
    try {
      ...
      console.log(`Getting product reviews for product: ${subcatId}, shop: ${shopOwnerId}`);
  ...
  async getAllReviews(c: Context) {
    try {
      const productReviews = await sql`SELECT * FROM customer_product_reviews ORDER BY created_at DESC`;
      const shopReviews = await sql`SELECT * FROM customer_shop_reviews ORDER BY created_at DESC`;
    ...
    try {
      ...
      const [updatedReview] = await sql`
        ...
        SET rating = ${body.rating || sql`rating`}, 
            comment = ${body.comment !== undefined ? body.comment : sql`comment`},
