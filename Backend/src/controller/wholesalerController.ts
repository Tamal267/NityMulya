...
export const addProductToInventory = async (c: any) => {
    try {
        ...
        const result = await sql`
            ...
            VALUES (${wholesaler_id}, ${subcat_id}, ${stock_quantity}, ${unit_price}, ${low_stock_threshold || 10})
...
export const updateInventoryItem = async (c: any) => {
    try {
        ...
        return c.json({ success: false, message: 'Failed to update inventory item' }, 500);
    }
};
...
export const getShopOrders = async (c: any) => {
    try {
        ...
        return c.json({ success: false, message: 'Failed to fetch shop orders' }, 500);
    }
};
